...
import { Search, Filter, Plus, Upload, Eye, CheckCircle, XCircle, Clock, MapPin, User, Phone, Mail, Award, BarChart3, AlertTriangle, Activity } from 'lucide-react';
        ...
        <CardContent>
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="flex-1 relative">
              ...
              <input type="text" placeholder="Search by name or location..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            </div>
            <select className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" value={riskFilter} onChange={e => setRiskFilter(e.target.value)}>
              <option value="all">All Risk Levels</option>
              <option value="low">Low Risk</option>
              <option value="medium">Medium Risk</option>
              <option value="high">High Risk</option>
            </select>
            <select className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}>
              <option value="all">All Statuses</option>
              <option value="active">Active</option>
              <option value="pending">Pending</option>
              <option value="inactive">Inactive</option>
            </select>
            <select className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" value={pageSize} onChange={e => setPageSize(Number(e.target.value))}>
          ...
          {/* Bulk Actions */}
          {selectedFarmers.length > 0 && <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex items-center justify-between">
          ...
          <div className="border rounded-lg overflow-hidden">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-12">
                    <input type="checkbox" checked={selectedFarmers.length === paginatedFarmers.length && paginatedFarmers.length > 0} onChange={handleSelectAll} className="rounded border-gray-300" />
                  </TableHead>
              ...
              <TableBody>
                {paginatedFarmers.map(farmer => <React.Fragment key={farmer.id}>
                    <TableRow className="hover:bg-gray-50">
                      <TableCell>
                        <input type="checkbox" checked={selectedFarmers.includes(farmer.id)} onChange={() => handleSelectFarmer(farmer.id)} className="rounded border-gray-300" />
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex space-x-1">
                          <Button size="sm" variant="outline" onClick={() => setExpandedFarmer(expandedFarmer === farmer.id ? null : farmer.id)}>
                            <Eye className="h-3 w-3 mr-1" />
                            {expandedFarmer === farmer.id ? 'Hide' : 'View'}
                          </Button>
                          <Button size="sm" variant="outline" onClick={() => setDetailView(detailView === farmer.id ? null : farmer.id)}>
                    ...
                    {expandedFarmer === farmer.id && <TableRow>
                        <TableCell colSpan={8}>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                              <div>
                                ...
                                <div className="space-y-2">
                                  {getFarmerDetails(farmer).riskFactors?.map((factor, index) => <div key={index} className="flex items-center justify-between p-2 bg-white rounded">
                                      <span className="text-sm">{factor}</span>
                                      <Badge variant="outline" className="text-xs">
                                        {farmer.risk === 'Low' ? 'Managed' : farmer.risk === 'High' ? 'Critical' : 'Monitored'}
                                      </Badge>
                                    </div>) || <div className="flex items-center justify-between p-2 bg-white rounded">
                              ...
                              <div>
                                ...
                                <div className="space-y-2">
                                  {getFarmerDetails(farmer).achievements?.map((achievement, index) => <div key={index} className="flex items-center p-2 bg-green-50 rounded">
                                      <CheckCircle className="h-4 w-4 text-green-600 mr-2" />
                                      <span className="text-sm text-green-800">{achievement}</span>
                                    </div>) || <div className="flex items-center p-2 bg-blue-50 rounded">
          ...
          <div className="mt-4 flex justify-between items-center">
            <div className="text-sm text-gray-500">
              ...
              {filteredFarmers.length !== data.farmers.length && <span className="text-blue-600 ml-2">(filtered from {data.farmers.length} total)</span>}
            </div>
            <Pagination>
              <PaginationContent>
                <PaginationItem>
