...
const Login = () => {
  ...
  const demoCredentials = [{
    ...
    password: 'farmer123',
    description: 'Constantine olives case study'
  }, {
    role: 'Farmer (Hamza)',
    email: 'hamza@radi.app',
    password: 'farmer123',
    description: 'Wheat hailstorm claim case study'
  }, {
    role: 'CNMA Insurer',
    email: 'cnma@radi.app',
    password: 'insurer123',
                ...
                {demoCredentials.map((cred, index) => <div key={index} className="text-xs">
                    <button type="button" onClick={() => handleDemoLogin(cred.email, cred.password)} className="text-left hover:text-agri-green transition-colors w-full">
                      <div className="flex flex-col">
                        ...
                        {cred.description && <span className="text-gray-500 italic">{cred.description}</span>}
                      </div>
                    </button>
                  </div>)}
              </div>
